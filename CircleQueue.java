...
public class CircleQueue {
  ...
  public void insertionSort() {
    ...
    while (node2 != null) {
      ...
      LinkedList key = new LinkedList(node2); // note: make key a different object, persistent/static in value (node2
                                              // moves)
  ...
  */
  public void selectionSort() {
    ...
    // continue while nodes remain in bounds
    while (node2 != null) {
      LinkedList currentMinNode = node1;
      LinkedList unsortedNode = node1;
      LinkedList tempNode = new LinkedList("null", null);
      LinkedList currentNode = node2;

      while (currentNode != null) {
        String minimumText = currentMinNode.getObject().toString();
        if (currentNode.getObject().toString().compareTo(minimumText) < 0)
          currentMinNode = currentNode;
        currentNode = currentNode.getNext();
      }

      // Swap current minimum nnode with the 1st unsorted node
      tempNode.setObject(currentMinNode.getObject());
      currentMinNode.setObject(unsortedNode.getObject());
      unsortedNode.setObject(tempNode.getObject());

      // Move current unsorted node to next unsorted node
      node1 = unsortedNode.getNext();
      // Move currentNode to 1st unsorted node
      node2 = node1.getNext();
    }
